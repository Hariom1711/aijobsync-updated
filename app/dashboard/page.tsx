...
import {
  Sparkles, Target, FileText, UploadCloud, BarChart3,
  LogOut, Crown, Rocket, ChevronRight, ShieldCheck,
  CheckCircle2, Plus, TrendingUp, Clock, Zap, Mic,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { useMasterProfile } from "@/hooks/useMasterProfile";
...
// ── quick action card ─────────────────────────────────────────────────────────
function QuickCard({
  title, desc, icon: Icon, iconColor, onClick, cta, badge,
}: {
  title: string; desc: string; icon: React.ElementType;
  iconColor: string; onClick: () => void; cta: string; badge?: string;
}) {
  return (
    <motion.div variants={item}>
      <Card
        className="h-full border-white/10 bg-white/8 backdrop-blur-xl hover:bg-white/12 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-purple-500/10 cursor-pointer"
        onClick={onClick}
      >
        <CardContent className="p-5 flex flex-col h-full">
          <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-purple-200/80 text-xs leading-relaxed flex-1">{desc}</p>
          <div className="mt-4 flex items-center gap-1 text-purple-300 text-xs font-medium group-hover:text-white">
...
// ── Pro feature nudge toast helper ────────────────────────────────────────────
function useProNudge() {
  const router = useRouter();
  return (feature: string) => {
    toast(`${feature} is a Pro feature ✨`, {
      description: "Upgrade to Pro for ₹1 — 7-day full access, cancel anytime.",
      action: {
        label: "Upgrade →",
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900">
          ...
          <div className="flex items-center gap-2">
            {isPro ? (
              <Button
                variant="outline"
                size="sm"
                className="bg-gradient-to-r from-amber-500/20 to-orange-500/20 border-amber-400/30 text-amber-200 hover:from-amber-500/30"
                onClick={() => router.push("/pro")}
              >
                <Crown className="h-3.5 w-3.5 mr-1.5" /> Pro Dashboard
              </Button>
            ) : (
              <Button
                variant="outline"
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-5 py-8 space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3">
            <div>
              ...
              <p className="text-purple-300 mt-1.5 text-sm">
                {isPro
                  ? "You're on Pro — all AI features unlocked. Let's get you hired."
                  : "Build job-specific resumes in seconds. Beat ATS. Land interviews."}
              </p>
            </div>
            <div className="flex items-center gap-2">
              {isPro && (
                <div className="flex items-center gap-1.5 text-xs text-amber-300 bg-amber-400/10 px-3 py-1.5 rounded-full border border-amber-400/20">
        ...
        {/* quick actions — Pro users see AI Interviewer card */}
        <motion.div
          variants={container}
          initial="hidden"
          animate="show"
        ...
        >
          ...
          <QuickCard
            ...
            icon={Mic}
            iconColor={isPro ? "text-purple-300" : "text-purple-300/50"}
            desc={isPro
              ? "Practice a full interview from your JD. Get AI feedback on every answer."
              : "Practice interviews from your real JD. Pro feature — try free for ₹1."}
            cta={isPro ? "Start Practice" : "Unlock with Pro"}
            badge={isPro ? undefined : "Pro"}
            onClick={() => isPro ? router.push("/interview") : router.push("/pro")}
          />
          <QuickCard
            title="My Resumes" icon={FileText} iconColor="text-emerald-300"
            desc="View, download, or regenerate your saved resumes."
        ...
        <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    >
                      ...
                      {r.score != null && (
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                          r.score >= 80 ? "bg-green-500/20 text-green-300"
                          : r.score >= 60 ? "bg-yellow-500/20 text-yellow-300"
          ...
          >
            ...
            <Card className="border-white/10 bg-white/8 backdrop-blur-xl">
              ...
              <CardContent className="space-y-2">
                {[
                  ...
                  ...(isPro
                    ? [
                        { label: "AI Interviewer",       path: "/interview" },
            ...
            {/* Pro upsell (free) / Pro status (pro) */}
            {isPro ? (
              <div className="rounded-2xl bg-gradient-to-br from-amber-500/15 via-orange-500/10 to-purple-500/15 border border-amber-400/30 p-5 backdrop-blur-xl">
                <div className="flex items-center gap-2 mb-3">
                  <Crown className="h-5 w-5 text-amber-300" />
                  <span className="text-white font-semibold text-sm">Pro Active</span>
                </div>
                <ul className="text-purple-200 text-xs space-y-1.5 mb-4">
                  {[
                    "Unlimited resumes",
                    "AI section rewriter ✨",
                    "AI Interviewer 🎤",
                ...
                <Button
                  className="w-full bg-gradient-to-r from-amber-400/30 to-orange-500/30 text-amber-200 border border-amber-400/30 text-sm hover:from-amber-400/40"
                  onClick={() => router.push("/pro")}
                >
                  Manage Pro Plan <ChevronRight className="h-3.5 w-3.5 ml-1" />
                </Button>
              </div>
            ) : (
              <div className="rounded-2xl bg-gradient-to-br from-amber-500/15 via-orange-500/10 to-pink-500/15 border border-amber-400/20 p-5 backdrop-blur-xl">
                ...
                <ul className="text-purple-200 text-xs space-y-1 mb-4">
                  {["Unlimited resumes", "AI rewriter ✨", "AI Interviewer 🎤", "Cover letters"].map((f) => (
                    <li key={f} className="flex items-center gap-1.5">
                      <CheckCircle2 className="h-3 w-3 text-amber-300 shrink-0" /> {f}
                    </li>
                  ))}
                ...
                <Button
                  ...
                  onClick={() => router.push("/pro")}
                >
                  Start for ₹1 — 7 days free <ChevronRight className="h-3.5 w-3.5 ml-1" />
                </Button>
              </div>
            )}
          </motion.div>
